  ...
  try {
    ...
    registerKeyHandler("s", async () => {
      if (!getRunningState()) {
        ...
        if (!isValid) {
          log("Session tokens are expired. Automatically re-authenticating...", "error");
          updateStatus("Expired tokens. Re-authenticating...", "error");
          render();
          ...
          if (privateKeys.length === 0) {
            ...
            log(`Found ${privateKeys.length} private keys. Authenticating...`, "info");
            await authenticateAllWallets(privateKeys);
            const newTokens = auth.readAllSessionTokensFromFile();
            if (newTokens.length === 0) {
              ...
              log(`Re-authentication successful! ${newTokens.length} accounts ready.`, "success");
              updateStatus(`${newTokens.length} accounts ready. Starting automation...`, "success");
              render();
              startAutomation();
              return;
            }
